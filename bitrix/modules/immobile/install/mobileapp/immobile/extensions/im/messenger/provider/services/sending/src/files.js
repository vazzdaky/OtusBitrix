/**
 * @module im/messenger/provider/services/sending/files
 */
jn.define('im/messenger/provider/services/sending/files', (require, exports, module) => {
	...
	class FilesUploadService
	{
		...
		#commitFiles(chatId, temporaryMessageId, filesIdsCollection, fromDisk = false)
		{
			...
			BX.rest.callMethod(RestMethod.imDiskFileCommit, {
				...
				.then(async (res) => {
					...
					await this.#updateRecentUploadingState(chatId, temporaryMessageId);
				})
				.catch(async (error) => {
		...
		/**
		 * @desc Update recent uploadingState by temporaryMessageId
		 * @param {number} chatId
		 * @param {string} temporaryMessageId
		 */
		async #updateRecentUploadingState(chatId, temporaryMessageId)
		{
			const dialogModel = this.store.getters['dialoguesModel/getByChatId'](chatId);
			const recentItem = this.store.getters['recentModel/getById'](dialogModel?.dialogId);

			if (recentItem?.uploadingState?.message?.id === temporaryMessageId)
			{
				recentItem.uploadingState.message.subTitleIcon = '';
				...
				logger.info(`${this.constructor.name}.updateRecentUploadingState element:`, recentItem);
			}
		}
