...
CREATE INDEX IF NOT EXISTS IX_B_HR_STRUCTURE_NODE_MEMBER_ROLE_ROLE_ID
	ON b_hr_structure_node_member_role (ROLE_ID);
...
CREATE TABLE IF NOT EXISTS b_hr_structure_node_settings
(
	ID BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
	NODE_ID BIGINT NOT NULL,
...
CREATE INDEX IF NOT EXISTS IX_B_HR_STRUCTURE_NODE_SETTINGS
	ON b_hr_structure_node_settings (NODE_ID, SETTINGS_TYPE);
...
CREATE INDEX IX_B_HR_STRUCTURE_NODE_PATH_CHILD_PARENT ON b_hr_structure_node_path (CHILD_ID, PARENT_ID);
...
CREATE TABLE IF NOT EXISTS b_hr_access_role (
	...
	CATEGORY VARCHAR(50) DEFAULT 'DEPARTMENT'
);
