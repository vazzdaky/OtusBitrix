...
if($arResult['ENABLE_NAVIGATION'])
{
	...
	if ($automationItems = \Bitrix\Crm\Automation\Helper::getNavigationBarItems(\CCrmOwnerType::ResolveID($arResult['DEFAULT_ENTITY_TYPE'])))
	{
		$navigationBar['ITEMS'] = array_merge($automationItems, $navigationBar['ITEMS']);
	}
	...
	if (\Bitrix\Crm\Integration\Calendar::isResourceBookingEnabled() && $arResult['PATH_TO_CALENDAR'])
	{
		$navigationBar['ITEMS'][] = array(
			'id' => 'calendar',
			'name' => GetMessage('CRM_WGT_FILTER_NAV_BUTTON_CALENDAR'),
			'active' => false,
			'url' => $arResult['PATH_TO_CALENDAR']
		);
	}
