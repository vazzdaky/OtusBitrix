/**
 * @module crm/entity-detail/toolbar/content/templates/audio-player-wrapper
 */
jn.define('crm/entity-detail/toolbar/content/templates/audio-player-wrapper', (require, exports, module) => {
	...
	class AudioPlayerWrapper extends LayoutComponent
	{
		/**
		...
		constructor(props)
		{
			...
			this.state = {
				...
				position: this.getPositionByValue(props.currentTime, props.duration),
				play: true,
				isTouchEnd: true,
			...
			this.updatePosition = throttle((position) => {
				this.setState({
					position,
					currentTime: this.getValueByPosition(position),
				});
			}, 25);
		...
		componentDidMount()
		{
			if (this.getProps().uid)
			{
				this.customEventEmitter = EventEmitter.createWithUid(this.getProps().uid);
		...
		{
			if (this.getProps().uid !== props.uid || this.props.uri !== props.uri)
			{
				this.removePlayerEvents();
		...
		getPositionByValue(value, duration)
		{
			if (!duration)
			{
				return 0;
			...
			return Math.floor(value * device.screen.width / duration);
		}

		setPosition(position)
		{
			position = Math.max(position, 0);
			position = Math.min(position, device.screen.width);
		...
		onPlay()
		{
			if (this.state.duration)
			{
				...
				this.customEventEmitter.emit('TopPanelAudioPlayer::onChangePlay', [{ uri }]);
			}
		}

		renderTrackInfo()
		{
			const currentTime = this.state.isTouchEnd
				? this.state.currentTime
				: this.getValueByPosition(this.state.position);
			...
			return View(
				...
				View(
					...
					Text({
						...
						text: String(this.convertSecondsToTime(currentTime)),
						onLayout: ({ width }) => {
					...
					onClick: () => {
						if (this.state.duration)
						{
							this.menu = new ContextMenu({
								...
								params: {
									title: Loc.getMessage('M_CRM_E_D_TOOLBAR_TITLE_AUDIO_PLAYER_SPEED'),
								},
							});
							this.menu.show();
						}
			...
			return View(
				{
					...
					onClick: () => {
						const { uri } = this.getProps();
						this.customEventEmitter.emit('TopPanelAudioPlayer::onCancel', [{ uri }]);
						this.hide();
					},
				},
				Image({
		...
		renderProgressBar()
		{
			...
			const leftOffsetByPosition = markerSize * this.state.position / device.screen.width;
					...
					View(
						{
							style: {
								height: this.state.isTouchEnd ? 3 : 11,
								width: this.state.position > device.screen.width / 2 ? (leftOffsetByPosition / 2) : 0,
							},
							clickable: false,
						},
					),
		...
		renderMarker()
		{
			...
			const markerOffset = {
				...
				left: this.state.position - leftOffsetByPosition,
			};
		...
		onTouchesEnded(x)
		{
			...
			const currentTime = this.getValueByPosition(this.state.position);
		...
		setPlayerSeek(currentTime)
		{
			...
			{
				const { uri } = this.getProps();
				this.customEventEmitter.emit('TopPanelAudioPlayer::onSetSeek', [{ currentTime, uri }]);
			}
		}
	}
